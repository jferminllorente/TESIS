...
\chapter{Ensayos con receptores comerciales}
	...
	Se desarrollan los materiales utilizados para las pruebas experimentales, principalmente haciendo hincapié en los receptores GNSS utilizados y su placa de evaluación. Por último, se muestran los resultados obtenidos en los ensayos.
...
\section{Esquema de ensayos con linea base nula}
...
\subfloat[Diplexor con desacople de continua.]{%
  \includegraphics[width=0.7\textwidth]{/EnsayosUblox/diplexor.png}
  \label{fig:diplexor}
}
\caption{Esquema de ensayo con rover y base estacionarios, para linea base nula.}
\end{figure}
	...
	Cómo el módulo ODIN no está siendo utilizado ya que no hay ninguna comunicación inalámbrica en este ensayo, puede ser deshabilitado mediante un puente en los terminales de arranque seguro del ODIN-W2 \cite{moving_base_AppN}. En cuanto a la interfaz cableada, se conecta el terminal de salida de la UART ($T_X$) de la base al terminal de entrada UART ($R_X$) del rover y se les da la misma referencia de tierra a ambas placas.
...
Scatter plot para GNSS y DGNSS.

ns me dice la cantidad de satélites...

sdX es la desviación estandar de los datos --> observar como se reduce drasticamente en el momento en que Q pasa de ser 2(Q=2 es float) a 1(Q=1 es fix).
Podría armar la matrix de covarianzas con sde,sdn,sdu,sdne,sdeu,sdun... Segun dice rtklib MANUAL...
